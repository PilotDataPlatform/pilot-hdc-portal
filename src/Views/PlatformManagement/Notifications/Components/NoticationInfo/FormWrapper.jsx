...
const FormWrapper = (props) => {
  ...
  const notificationFormSubmit = async (values) => {
    ...
    if (!edit) {
      try {
        setLoading(true);
        const res = await createNotification(values.message, detail);
        if (res.data) {
          const activeNotification = mapToNewStructure(res.data);
          dispatch(
          ...
          dispatch(
            notificationActions.setActiveNotification(activeNotification),
          );
    ...
    const updatedData = {
      ...
      detail: {
        ...
        maintenanceDate: allValues['date']
          ? moment(allValues['date']).utc().format('YYYY-MM-DDTHH:mm:ssZ')
        ...
        initialValues={
          edit
            ? {
                ...
                date: moment(activeNotification.detail.maintenanceDate),
      ...
      >
        ...
        <div className={styles.input2}>
          ...
          {edit ? (
            ...
            <div className={styles['create-notification__header-buttons']}>
              <Form.Item>
                <Button
                  type="text"
                  onClick={() =>
                    dispatch(
                      notificationActions.setCreateNewNotificationStatus(false),
                    )
                  }
