...
import styles from './CopyDataToCoreRequest.module.scss';
...
const CopyDataToCoreRequest = (props) => {
  ...
  async function resetList() {
    ...
    dispatch(request2CoreActions.setReqList([]));
  ...
  const handleMarkAsRequest = async () => {
    try {
      ...
      } else if (res.data.result?.pendingCount === 0) {
              ...
              <>
                <div className={styles.right_content_header}>
                  <div className={styles.header_left_part}>
                    ...
                    <p>
                      <b>Destination:</b> {`Core/` + activeReq.destinationPath}
                    </p>
                    {status === 'complete' ? (
                      <p>
                        <b>Review Note:</b>{' '}
...
export default CopyDataToCoreRequest;
