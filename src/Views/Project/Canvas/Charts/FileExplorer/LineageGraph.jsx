...
import { updateLineage } from '../../../../../Utility/lineage/updateLineage';
...
export default function LineageGraph({
  ...
  useEffect(() => {
    ...
    async function loadLineage() {
      setIsLoading(true);
      const recordWithLineage = await updateLineage(record, direction);
      setIsLoading(false);
      const lineage = recordWithLineage?.lineage;
      if (lineage) {
        lineage.nodeLabel = recordWithLineage?.nodeLabel;
      }
      ...
      for (let i = 0; i < nodeKeys.length; i++) {
        ...
        let location;
        let fileType;
        let isCurrentNode = nodeInfo.guid === recordWithLineage.baseEntityGuid;
      ...
      const tooltip = new G6.Tooltip({
        ...
        getContent: (e) => {
          ...
          outDiv.innerHTML = `
          <h4>${
            e.item.getModel().isPipeline ? 'Pipeline Info' : 'Node Info'
          }</h4>
      ...
      if (nodes.length > 0) {
        ...
        let config = {
          ...
          height: 500,
          plugins:
            toolBarNotSupported || !showFitView
              ? [tooltip]
              : [toolbar, tooltip],
          fitCenter: true,
          defaultNode: {
