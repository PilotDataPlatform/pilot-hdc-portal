    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (res.data.result.ignored && res.data.result.ignored.length > 0) {
          ...
          res.data.result.processing.forEach((currentMove) => {
            dispatch(
              datasetFileOperationsCreators.setMove({
                globalEntityId: currentMove.id,
                datasetCode: currentMove.containerCode,
                name: currentMove.name,
                fileSize: currentMove.size,
                operator: username,
